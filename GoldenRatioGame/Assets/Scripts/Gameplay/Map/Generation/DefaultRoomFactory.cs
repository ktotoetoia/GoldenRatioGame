...
namespace IM.Map
{
   [CreateAssetMenu(menuName = "Map/Room Factories/Rectangle Room Factory")]
    public class DefaultRoomFactory : RoomFactory
    {
        ...
        {
            List<IRoomPattern> patterns = new();
            RectangleRoomPatternFactory patternFactory = new();

            foreach (Vector2Int roomSize in _roomSizes)
                patterns.Add(patternFactory.Create(_size, roomSize));

            patterns.Add(new OffsetRoomPatternFactory().Create(_size,
                new List<Vector2Int> { new(0, 0), new(1, 0), new(1, 1) }));
        ...
        public override IRoom Create(ISelectedRoomPattern roomPattern, IGameObjectFactory gameObjectFactory)
        {
            ...
            if (roomGO.TryGetComponent(out IRoomForm form)) form.Apply(roomPattern.Shape);

            foreach (var (cellOffset, ports) in roomPattern.PortDefinitions)
            foreach (IPortDefinition portDefinition in ports)
            {
                RoomPort port = gameObjectFactory.Create(_roomPortPrefab, false).GetComponent<RoomPort>();
