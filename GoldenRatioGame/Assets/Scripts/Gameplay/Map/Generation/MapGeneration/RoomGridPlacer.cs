...
namespace IM.Map.Grid
{
    public class RoomGridPlacer
    {
        ...
        public void Place(IGrid<CellInfo> grid, IRoomFactory factory, Vector2Int position)
        {
            grid[position] = new CellInfo(PickPattern(factory), factory);
        }

        public bool PlaceClose(IGrid<CellInfo> grid, IRoomFactory factory)
        {
            List<Vector2Int> available = GetAvailablePositions(grid);
            if (available.Count == 0) return false;

            available = available.OrderBy(_ => _random.Next()).ToList();
            List<IRoomPattern> patterns = factory.GetRoomPatterns().OrderBy(_ => _random.Next()).ToList();
        ...
        private List<Vector2Int> GetAvailablePositions(IGrid<CellInfo> grid)
        {
            ...
            for (int x = 0; x < grid.Width; x++)
            {
                for (int y = 0; y < grid.Height; y++)
                {
                    ...
                    if (HasOccupiedNeighborPointingHere(grid, pos)) positions.Add(pos);
                }
            }
        ...
        private bool HasOccupiedNeighborPointingHere(IGrid<CellInfo> grid, Vector2Int pos)
        {
        ...
        private bool HasPortTowardAnyNeighbor(IGrid<CellInfo> grid, IRoomPattern pattern, Vector2Int pos)
        {
            foreach (Vector2Int dir in PortSideUtility.Directions)
            {
                Vector2Int neighborPos = pos + dir;
        ...
        private bool HasPort(IRoomPattern pattern, PortSide side) =>
            pattern.RequiredPortDefinitions.Concat(pattern.OptionalPortDefinitions).Any(p => p.Side == side);
