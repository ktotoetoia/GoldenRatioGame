...
namespace IM.Modules
{
    ...
    {
        [SerializeField] private List<TagDirectionPortInfo>  _portInfos;
        
        public override void Initialize(IList<IPort> ports,IDictionary<IPort, IPortInfo> portsDictionary,  IModule module)
        {
            if(ports.Count != 0) throw new ArgumentException("Ports must be empty to initialize");
            
            foreach (TagDirectionPortInfo portInfo in _portInfos)
            {
                ...
                ports.Add(port);
                portsDictionary.Add(port, portInfo);
            }
        }
    }
}